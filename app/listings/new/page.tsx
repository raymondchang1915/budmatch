...
async function updateMarketPrice(model: string, userPrice: number) {
  ...
  if (data) {
    ...
    await supabase.from('model_prices').update({ market_price: newAvg, submission_count: data.submission_count + 1, updated_at: new Date().toISOString() }).eq('model', model)
  } else {
...
export default function NewListing() {
  const [form, setForm] = useState({
    user_email: '', model: '', has_component: '', needs_component: '',
    condition: '', location: '', listing_type: 'both', asking_price: '',
  })
  const [marketPrice, setMarketPrice] = useState<number | null>(null)
  const [priceBounds, setPriceBounds] = useState<{ suggestedPrice: number, maxPrice: number, minPrice: number, demandRatio: number } | null>(null)
  ...
  const handleSubmit = async () => {
    ...
    await supabase.from('demand_stats').upsert([{ model: form.model, component: form.has_component, have_count: 1 }], { onConflict: 'model,component' })
    const { data: listing, error: insertError } = await supabase.from('listings').insert([{
      user_email: form.user_email, model: form.model, has_component: form.has_component,
      needs_component: form.needs_component, condition: form.condition, location: form.location,
      listing_type: form.listing_type, asking_price: askingPrice || null,
    }]).select().single()
    if (insertError) { setError(insertError.message); setLoading(false); return }
    const { data: potentialMatches } = await supabase.from('listings').select('*')
      .eq('model', form.model).eq('has_component', form.needs_component)
      .eq('needs_component', form.has_component).eq('matched', false).neq('user_email', form.user_email)
    if (potentialMatches && potentialMatches.length > 0) {
      const bestMatch = potentialMatches[0]
      await supabase.from('matches').insert([{ listing_a: listing.id, listing_b: bestMatch.id, status: 'pending' }])
      ...
      <div className="relative max-w-lg mx-auto px-6 py-16">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div>
            ...
            <select className={selectClass} value={form.model} onChange={e => handleModelChange(e.target.value)}>
              <option value="">Select a model</option>
              {EARBUD_MODELS.map(m => <option key={m}>{m}</option>)}
            </select>
            {marketPrice && <p className="text-xs text-gray-400 mt-1.5 ml-1">Market price: LKR {marketPrice.toLocaleString()}</p>}
          </div>

          <div>
            <label className={labelClass}>I have</label>
            <select className={selectClass} value={form.has_component}
              onChange={e => handleComponentChange(e.target.value)} disabled={!form.model}>
          ...
          <div>
            ...
            <select className={selectClass} value={form.needs_component}
              onChange={e => setForm({ ...form, needs_component: e.target.value })} disabled={!form.has_component}>
              <option value="">What do you need?</option>
              {availableNeeds.map(c => <option key={c}>{c}</option>)}
            </select>
            {form.has_component && <p className="text-xs text-gray-400 mt-1.5 ml-1">Showing compatible components only</p>}
          </div>
          ...
          <div>
            <label className={labelClass}>
              Asking price (LKR)
              {priceBounds && <span className="text-gray-400 font-normal ml-2">LKR {priceBounds.minPrice.toLocaleString()} – {priceBounds.maxPrice.toLocaleString()}</span>}
            </label>
            <input type="number"
              placeholder={!form.model ? 'Select a model first' : !form.has_component ? 'Select component first' : priceBounds ? `Suggested: LKR ${priceBounds.suggestedPrice.toLocaleString()}` : 'Loading...'}
              className={`${inputClass} ${priceError ? 'border-red-400 ring-red-400' : ''}`}
              value={form.asking_price} onChange={e => handlePriceChange(e.target.value)} disabled={!priceBounds} />
            {priceError && <p className="text-red-500 text-xs mt-1.5 ml-1">{priceError}</p>}
            {priceBounds && !priceError && <p className="text-xs text-gray-400 mt-1.5 ml-1">{Math.round(priceBounds.demandRatio * 100)}% demand ratio right now</p>}
          </div>
